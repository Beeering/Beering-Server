/**
 * Created by jisooyoon on 2017. 4. 19..
 */
...
prediction.config.APP_HOST = 'http://localhost:7070';
...
describe('User', function() {
    describe('getUser', function () {
        before(function () {
            nock('http://localhost:7070').get('/users/1.json?pio_appkey=key').reply(200, {uid: 1});
            nock('http://localhost:7070').get('/users/2.json?pio_appkey=key').reply(400, {
                errors: [{
                    field: 'pio_appkey',
    ...
    describe('createUser', function () {
        before(function () {
            nock('http://localhost:8000').post('/users.json', {
                pio_appkey: 'key',
                pio_uid: 1
            ...
            nock('http://localhost:8000').post('/users.json', {
                pio_appkey: 'key',
                pio_uid: 2
            }).reply(400, {errors: [{field: 'pio_appkey', message: 'This field is required'}]});
        });
